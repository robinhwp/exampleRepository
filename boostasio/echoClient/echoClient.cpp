...
const unsigned short PORT_NUMBER = 13400;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (connect_error)
	{
		std::cout << "연결실패 error No: " << connect_error.value() << 
			" error Message: " << connect_error.message() << std::endl;
	...
	for (int i = 0; i < 7; ++i)
	{
		...
		sprintf_s(szMessage, 128 - 1, "%d - Send Message", i+1);
		...
		std::cout << "서버에서 받은 메시지: " << &buf[0] << std::endl;
	}
